...
class ReportModel {
    ...
    create = async ({ reportName, orgId, userId, reportConfig }) => {
        const sql = `INSERT INTO ${this.tableName}
        (reportName,orgId,userId,reportConfig) VALUES (?,?,?,?)`;

        console.log("Create Quesry>>>>>> ", sql)
        const result = await query(sql, [reportName, orgId, userId, reportConfig]);
    ...
    update = async (params, id) => {
        const { columnSet, values } = multipleColumnSet(params)
        const sql = `UPDATE reports SET ${columnSet} WHERE id = ?`;
