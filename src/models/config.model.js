...
class ConfigModel {
    ...
    create = async ({ configName, configValue, configStatus, orgId }) => {
        const sql = `INSERT INTO ${this.tableName}
        (configName,configValue,configStatus,orgId) VALUES (?,?,?,?)`;

        console.log("Create Quesry>>>>>> ", sql)
        const result = await query(sql, [configName, configValue, configStatus, orgId]);
    ...
    update = async (params, id) => {
        const { columnSet, values } = multipleColumnSet(params)
        const sql = `UPDATE config SET ${columnSet} WHERE id = ?`;
